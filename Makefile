...
release-prep: ## Trigger release prep workflow.
	@if [ -z "$(VERSION)" ] || [ -z "$(BRANCH)" ]; then echo "Usage: make release-prep VERSION=v1.3.0 BRANCH=release/v1.3"; exit 1; fi
...
generate-metricsdocs: ## Generate metrics reference documentation.
	...
	go run ./internal/metrics/metricsdocs > docs/generated/metrics.md
