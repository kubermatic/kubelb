...
| `persistence` _[LoadBalancerPersistence](#loadbalancerpersistence)_ | Persistence configures backend endpoint persistence. When omitted,<br />KubeLB keeps the default non-sticky load balancing behavior.<br />SourceIP persistence is based on the source IP observed by KubeLB Envoy<br />for TCP and UDP traffic, which may be a gateway, node, or NAT address in<br />proxied topologies. |  | Optional: \{\} <br /> |
