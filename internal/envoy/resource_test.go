...
func TestMapSnapshot_VersionIsDeterministic(t *testing.T) {
	...
	snap1, err := MapSnapshot(ctx, cl, lbs, routes, pa, "test-node")
	if err != nil {
		t.Fatalf("MapSnapshot #1: %v", err)
	}
	snap2, err := MapSnapshot(ctx, cl, lbs, routes, pa, "test-node")
	if err != nil {
...
func TestMapSnapshot_ProducesExpectedResources(t *testing.T) {
	...
	snap, err := MapSnapshot(ctx, cl, lbs, nil, pa, "test-node")
	if err != nil {
...
func TestMapSnapshot_VersionStableAcrossReorder(t *testing.T) {
	...
	snap1, err := MapSnapshot(ctx, cl, lbs, nil, pa, "test-node")
	if err != nil {
		t.Fatalf("MapSnapshot #1: %v", err)
	}
	...
	snap2, err := MapSnapshot(ctx, cl, lbs2, nil, pa, "test-node")
	if err != nil {
		t.Fatalf("MapSnapshot #2: %v", err)
	}
